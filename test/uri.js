...
describe('URI', function() {
  ...
  it('parses uri strings correctly (test vector)', function() {
    ...
    uri = URI.parse('telestai:');
    expect(uri.address).to.be.undefined();
    expect(uri.amount).to.be.undefined();
    expect(uri.otherParam).to.be.undefined();

    uri = URI.parse('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K');
    uri.address.should.equal('R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K');
    expect(uri.amount).to.be.undefined();
    expect(uri.otherParam).to.be.undefined();

    uri = URI.parse('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=123.22');
    uri.address.should.equal('R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K');
  ...
  it('URIs can be validated statically (test vector)', function() {
    URI.isValid('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K').should.equal(true);
    URI.isValid('telestai:moSNtynymW8y9Pe4veLGhSpBL8yw4t8JWs').should.equal(true);

    URI.isValid('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=1.2')
                .should.equal(true);
    URI.isValid('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=1.2&other=param')
                .should.equal(true);
    URI.isValid('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=1.2&req-other=param',
                ['req-other']).should.equal(true);
    URI.isValid('telestai:RDWmJMh9iBvwbvgohRCynzoyP3ojq8st63?amount=0.1&' +
                'r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu').should.equal(true);

    URI.isValid('telestai:').should.equal(false);
    URI.isValid('telestai:badUri').should.equal(false);
    URI.isValid('telestai:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfk?amount=bad').should.equal(false);
    URI.isValid('telestai:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfk?amount=1.2&req-other=param')
                .should.equal(false);
    URI.isValid('telestai:?r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu')
                .should.equal(false);
  });
  ...
  it('do not need new keyword', function() {
    var uri = URI('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K');
    uri.should.be.instanceof(URI);
  });

  describe('instantiation from telestai uri', function() {
    /* jshint maxstatements: 25 */
    var uri;

    it('parses address', function() {
      uri = new URI('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K');
      uri.address.should.be.instanceof(telestaicore.Address);
      uri.network.should.equal(Networks.livenet);
    });

    it('parses amount', function() {
      uri = URI.fromString('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=123.22');
    ...
    it('parses a testnet address', function() {
      uri = new URI('telestai:mp5vhtKLjEB2BSJQ5GJ5e399jDDgWpFAmb');
      uri.address.should.be.instanceof(telestaicore.Address);
      uri.network.should.equal(Networks.testnet);
    });

    it('stores unknown parameters as "extras"', function() {
      uri = new URI('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=1.2&other=param');
      uri.address.should.be.instanceof(telestaicore.Address);
    ...
    it('throws error when a required feature is not supported', function() {
      (function() {
        return new URI('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=1.2&other=param&req-required=param');
      }).should.throw(Error);
    });
  ...
  it('should support double slash scheme', function() {
    var uri = new URI('telestai://R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K');
    uri.address.toString().should.equal('R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K');
  });

  it('should input/output String', function() {
    var str = 'telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?' +
  ...
  it('should support numeric amounts', function() {
    var uri = new URI('telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=12.10001');
  ...
  it('should generate a valid URI', function() {
    new URI({
      ...
      'telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K'
    );
    ...
    new URI({
      ...
      'telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K?amount=1.10001&message=Hello%20World&something=else'
    );
  ...
  it('writes correctly the "r" parameter on string serialization', function() {
    var originalString = 'telestai:RDWmJMh9iBvwbvgohRCynzoyP3ojq8st63?amount=0.1&' +
  ...
  it('displays nicely on the console (#inspect)', function() {
    var uri = 'telestai:R9wprka91ffCccLXPTbLpFf8vCoTPTkh4K';
